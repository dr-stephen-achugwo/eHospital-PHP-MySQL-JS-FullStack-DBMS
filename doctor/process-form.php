...
<?php
    ...
    if (isset($_POST['submit'])) 
    {
        ...
        $patientQuery = "SELECT patient_id FROM ehos_patient WHERE patient_name = '$patientName'";
        $diagnosisQuery = "SELECT diagnosis_id FROM ehos_diagnoses WHERE diagnosis_name = '$diagnosisName'";
        $medicationQuery = "SELECT medication_id FROM ehos_medications WHERE medication_name = '$medicationName'";
        ...
        if ($patientResult && $diagnosisResult && $medicationResult) 
        {
            ...
            $sql = "INSERT INTO ehos_prescriptions (patient_id, diagnosis_id, medication_id, prescription_date, doctor_id) VALUES ('$patientId', '$diagnosisId', '$medicationId', '$prescriptionDate', '$doctorId')";
            if ($database->query($sql)) {
                echo '<div style="display:flex;justify-content: center;align-items: center;width: 100%;height: 50px;background-color: #4CAF50;color: white;font-size: 20px;"><strong> Successfully added!</strong> Your recipe has been successfully saved! </div>';
            } else {
